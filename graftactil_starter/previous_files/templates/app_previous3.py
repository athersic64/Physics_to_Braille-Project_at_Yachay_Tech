...
def calcular_limites(fig_size_mm, paso_mm, tick_step):
    ...
    if rango_x == 0: rango_x = max(1.0, tick_step * 2)
    if rango_y == 0: rango_y = max(1.0, tick_step * 2)
    return (-rango_x, rango_x), (-rango_y, rango_y)

def make_callable_from_expr(expr_str: str):
    """
    Parse user expression to a numpy-callable function using sympy safe-ish parsing.
    Accepts expressions in 'x' using standard math functions (sin, cos, exp, etc.).
    """
    x = sp.symbols('x')
    try:
        expr = sp.sympify(expr_str, evaluate=True)
        fn = sp.lambdify(x, expr, modules=["numpy"])
        # quick test with scalar to catch obvious errors
        try:
            _ = fn(0.1)
        except Exception:
            # try alternative: allow 'np.' in input by replacing with math names
            fn = sp.lambdify(x, expr, modules=["numpy"])
        return fn
    except Exception as e:
        raise ValueError(f"Failed to parse expression '{expr_str}': {e}")

def build_3d_from_config(config: dict):
    ...
    # base plate centered at origin, top face at z=plate_thickness
...
async def generate(request: Request, background_tasks: BackgroundTasks):
    ...
    # Validate marker arrays lengths (simple)
    nfunc = len(funcs)
    keys_to_check = ['marker_segments', 'marker_densities', 'marker_shapes', 'marker_sizes']
